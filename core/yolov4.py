...
def decode(conv_output, w_size, h_size, NUM_CLASS, STRIDES, ANCHORS, i, XYSCALE=[1,1,1], FRAMEWORK='tf'):
    return decode_tf(conv_output, w_size, h_size, NUM_CLASS, STRIDES, ANCHORS, i=i, XYSCALE=XYSCALE)
...
def decode_tf(conv_output, output_w, output_h, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    batch_size = tf.shape(conv_output)[0]
    conv_output = tf.reshape(conv_output,
                             (batch_size, output_w, output_h, 3, 5 + NUM_CLASS))

    conv_raw_dxdy, conv_raw_dwdh, conv_raw_conf, conv_raw_prob = tf.split(conv_output, (2, 2, 1, NUM_CLASS),
                                                                          axis=-1)
    xy_grid = tf.meshgrid(tf.range(output_h), tf.range(output_w))
    xy_grid = tf.expand_dims(tf.stack(xy_grid, axis=-1), axis=2)  # [gx, gy, 1, 2]
